...
router.get('/', async (req, res) => {
  try {
    ...
    // Filter by mood
    if (mood) {
      const moodLower = (mood as string).toLowerCase().trim();
      if (moodLower === 'gaming') {
        where.OR = [
    ...
    const allCategories = [...new Set((await prisma.product.findMany({ select: { category: true } })).map(p => p.category))].sort();
...
router.post('/track', async (req, res) => {
  try {
    ...
    const log = await prisma.userActivity.create({
      data: {
        userId: userId || null,
        action,
        targetId,
