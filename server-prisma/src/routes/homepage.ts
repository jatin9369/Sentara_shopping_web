...
router.get('/layout', async (req, res) => {
  try {
    ...
    let sections = await prisma.homepageLayout.findMany({
      orderBy: { order: 'asc' }
    });
    ...
    if (sections.length === 0) {
      ...
      await prisma.homepageLayout.createMany({ data: defaultSections });
      sections = await prisma.homepageLayout.findMany({ orderBy: { order: 'asc' } });
    }
...
router.post('/layout/reorder', async (req, res) => {
  try {
    ...
    const updated = await prisma.homepageLayout.findMany({ orderBy: { order: 'asc' } });
